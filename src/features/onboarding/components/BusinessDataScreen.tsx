  ...
  return (
    <SafeAreaView style={styles.screen}>
      ...
      >
        ...
        <View style={styles.form}>
          ...
          <View style={styles.field}>
            ...
            <RadioItem
              label="Tenho somente uma matriz"
              selected={!hasBranches}
              onPress={() => {
            ...
            />
            <RadioItem
              label="Tenho filiais em cidades da Ibiapaba"
              selected={hasBranches}
              onPress={() => setHasBranches(true)}
            />
            <PickerField
              label="Cidade da matriz"
              value={headquartersCity?.name}
              onPress={() => setPicker('headquarters')}
            />
            {hasBranches ? (
              <PickerField
                label="Cidades com filiais"
                value={
                  branchCities.length ? branchCities.map((city) => city.name).join(', ') : undefined
                }
                onPress={() => setPicker('branches')}
              />
            ) : null}
          </View>
          ...
          {cities.isError ? (
            <Text style={styles.error}>Não foi possível carregar as cidades.</Text>
          ) : null}
        </View>
      </ScrollView>
      ...
      <CityPicker
        ...
        selected={picker === 'branches' ? branchCities : headquartersCity ? [headquartersCity] : []}
        visible={picker !== null}
      />
    </SafeAreaView>
  );
...
function CityPicker({
  ...
  multiple: boolean;
  onClose: () => void;
  onSelect: (cities: OnboardingCity[]) => void;
  selected: OnboardingCity[];
  visible: boolean;
}) {
        ...
        <View style={styles.sheet}>
          <View style={styles.sheetHeader}>
            <Text style={styles.sheetTitle}>
              {multiple ? 'Cidades com filiais' : 'Cidade da matriz'}
            </Text>
          ...
          <ScrollView>
            {cities.map((city) => {
              const selectedCity = selectedIds.has(city.id);
              return (
                <Pressable key={city.id} onPress={() => toggle(city)} style={styles.cityRow}>
          ...
          {multiple ? (
            <Pressable onPress={() => onSelect(draft)} style={styles.primaryButton}>
